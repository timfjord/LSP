...
def create_code_action_edit(view: sublime.View, version: int, edits: List[Tuple[str, Range]]) -> Dict[str, Any]:
    return {
        "documentChanges": [
            {"textDocument": versioned_text_document_identifier(view, version), "edits": list(map(edit_to_lsp, edits))}
        ]
    }
...
def create_test_code_action(
    view: sublime.View, version: int, edits: List[Tuple[str, Range]], kind: str = None
) -> Dict[str, Any]:
    action = {"title": "Fix errors", "edit": create_code_action_edit(view, version, edits)}
    if kind:
        action['kind'] = kind
    return action


def create_test_code_action2(
    command_name: str, command_args: Optional[List[Any]] = None, kind: str = None
) -> Dict[str, Any]:
    action = {"title": "Fix errors", "command": create_command(command_name, command_args)}
    if kind:
        action['kind'] = kind
    return action
...
def create_test_diagnostics(diagnostics: List[Tuple[str, Range]]) -> Dict:
    def diagnostic_to_lsp(diagnostic: Tuple[str, Range]) -> Dict:
        ...
        return {"message": message, "range": range.to_lsp()}

    return {"uri": TEST_FILE_URI, "diagnostics": list(map(diagnostic_to_lsp, diagnostics))}
...
class CodeActionsOnSaveTestCase(TextDocumentTestCase):
    ...
    def test_applies_matching_kind(self) -> Generator:
        ...
        code_action = create_test_code_action(
            self.view, self.view.change_count(), [(';', Range(Point(0, 11), Point(0, 11)))], code_action_kind
        )
...
class CodeActionsTestCase(TextDocumentTestCase):
    ...
    def test_requests_code_actions_on_newly_published_diagnostics(self) -> Generator:
        ...
        yield from self.await_client_notification(
            "textDocument/publishDiagnostics",
            create_test_diagnostics(
                [('issue a', Range(Point(0, 0), Point(0, 1))), ('issue b', Range(Point(1, 0), Point(1, 1)))]
            ),
        )
        params = yield from self.await_message('textDocument/codeAction')
        print(params)
    ...
    def test_runs_command_in_resolved_code_action(self) -> Generator:
        ...
        resolved_code_action["edit"] = create_code_action_edit(
            self.view,
            3,
            [
                ("c", Range(Point(0, 0), Point(0, 1))),
